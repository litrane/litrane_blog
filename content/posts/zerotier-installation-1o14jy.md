...
tags:
  ...
  - 网络配置
  - 安装脚本
keywords: 文件存储,docker,自托管,网络配置,安装脚本
description: >-
  该文介绍了使用bash脚本在自托管环境中安装zerotier客户端，通过检测操作系统类型和分发版进行合适的安装操作，提供了针对各类Unix-like系统的快速安装指南，包括MacOS、Debian、RedHat/CentOS、Fedora、SuSE和Mint
  Linux等。
isCJKLanguage: true
---
...
[https://bashupload.com/](https://bashupload.com/)

use https://github.com/xubiaolin/docker-zerotier-planet to self-host zerotier server

bash file that installs docker and docker-compose
...
bash file that installs zerotier client

```python
curl -s https://install.zerotier.com | sudo bash  # Linux系统
```

bash file that binds ip address to k8s config file

```python
#!/bin/bash

# 获取ztmjfjsueh网卡的IP地址
IP=$(ifconfig ztmjfjsueh | grep 'inet ' | awk '{ print $2}')
...
​![image](http://127.0.0.1:6806/assets/image-20230722101127-7ny85rx.png)​

​![image](http://127.0.0.1:6806/assets/image-20230722101139-rr55kbs.png)​

‍

bash file that installs zerotier client in self-host environment
...
$SUDO  wget http://bashupload.com/lrF5Y/planet
$SUDO service zerotier-one restart
$SUDO  zerotier-cli join 40276f5cdd062ca9
echo
...
​![image](http://127.0.0.1:6806/assets/image-20230722102635-dnqgu25.png)​
